...
src = r'\\fatherserverdw\Q\research\images\clue_batch2\great'
dst = os.path.join(src,'prediction')
imlist = glob(os.path.join(src,'*.ndpi'))

# Load image
for impth in imlist:
    base,imnm = os.path.split(impth)
    if os.path.exists(os.path.join(dst, imnm.replace('.ndpi', '.png'))): continue

    # imobj = Image.open(os.path.join(src,imnm))
    imobj = OpenSlide(os.path.join(src,imnm))
